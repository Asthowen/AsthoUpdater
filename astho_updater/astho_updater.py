...
class AsthoUpdater(object):
    def __init__(
            ...
            logger_name: str = 'AsthoUpdater',
            logger_date_format: str = '%H:%M:%S - %d/%m/%Y',
        ...
        self.__logger = Logger(logger_name, logger_status, logger_date_format)
    ...
    @staticmethod
    async def __limit_tasks(number: int, *tasks):
    ...
    async def __get_hash(self, file_path: str) -> str:
        async with aiofiles.open(file_path, 'rb') as f:
            ...
            if self.__algorithm == 'sha1':
                return hashlib.sha1(content).hexdigest()
            elif self.__algorithm == 'sha224':
                return hashlib.sha224(content).hexdigest()
            elif self.__algorithm == 'sha256':
                return hashlib.sha256(content).hexdigest()
            elif self.__algorithm == 'sha384':
        ...
        if self.__file_to_download > 1:
            if self.__json_content['maintenance'] != 'on':
                ...
                if self.__file_downloaded:
                    ...
                    self.__logger.log('Nothing to download.')
            else:
